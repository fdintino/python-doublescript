...
def add_subclass(base, cls):
    ...
    subclasses_type = dict if seven.PY3 else list
    ...
    if seven.PY3:
        ...
        for i in seven.moves.range(len(subclasses) - 1, -1, -1):
...
def force_heap_type(cls):
    """Ensures that the type has the HEAPTYPE flag set within its context"""
    c_cls = c_typeobj(cls)
...
wrapper_descriptor = type(type.__init__)
